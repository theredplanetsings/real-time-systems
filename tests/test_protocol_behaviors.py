...
def _task(
    task_id: int, period: int, computation: int, resources: dict[str, int], phase: int = 0
) -> TaskSpec:
    return TaskSpec(
        task_id=task_id,
        phase=phase,
...
def test_none_protocol_can_block_on_resource_contention() -> None:
    ...
    segments = simulate_uniprocessor(
        tasks, horizon=8, algorithm="RM", protocol="None", resource_order="Resources then CPU"
    )
...
def test_npp_marks_non_preemptive_resource_phase_as_resource() -> None:
    ...
    segments = simulate_uniprocessor(
        tasks, horizon=8, algorithm="RM", protocol="NPP", resource_order="Resources then CPU"
    )
...
def test_pcp_executes_resource_phases_without_crashing() -> None:
    ...
    segments = simulate_uniprocessor(
        tasks, horizon=8, algorithm="DM", protocol="PCP", resource_order="Resources then CPU"
    )
...
def test_pip_runs_without_crashing_and_emits_segments() -> None:
    ...
    segments = simulate_uniprocessor(
        tasks, horizon=9, algorithm="RM", protocol="PIP", resource_order="Resources then CPU"
    )
    assert len(segments) > 0
