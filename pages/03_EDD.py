...
st.set_page_config(page_title="EDD", layout="wide")

st.title("EDD Scheduling")

protocol = render_sidebar("EDD", show_protocol=True)
...
render_schedulability(rows, "EDD")
...
if st.button("Generate EDD schedule"):
    ...
    segments = simulate_uniprocessor(rows, int(range_end), "EDD", protocol or "None", resource_order)
