...
st.set_page_config(page_title="Partitioned DM", layout="wide")

st.title("Partitioned DM Scheduling")

render_sidebar("Partitioned DM", show_protocol=True)
...
rows = render_task_inputs(
    ...
    key_prefix="partitioned_dm",
)

st.subheader("Schedulability")
render_schedulability(rows, "Partitioned DM", processors=int(processors))
...
if st.button("Generate Partitioned DM schedule"):
    ...
    if png is None:
        ...
        st.download_button(
            ...
            data=png,
            file_name="partitioned_dm_schedule.png",
            mime="image/png",
        )
