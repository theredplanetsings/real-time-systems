...
st.set_page_config(page_title="EDF", layout="wide")

st.title("EDF Scheduling")

protocol = render_sidebar("EDF", show_protocol=True)
...
render_schedulability(rows, "EDF")
...
if st.button("Generate EDF schedule"):
    ...
    segments = simulate_uniprocessor(rows, int(range_end), "EDF", protocol or "None", resource_order)
