...
st.set_page_config(page_title="Global RM", layout="wide")

st.title("Global RM Scheduling")

render_sidebar("Global RM", show_protocol=True)
...
rows = render_task_inputs(
    ...
    key_prefix="global_rm",
)

st.subheader("Schedulability")
render_schedulability(rows, "Global RM", processors=int(processors))
...
if st.button("Generate Global RM schedule"):
    ...
    segments = simulate_global_rm(rows, int(range_end), int(processors))
    ...
    if png is None:
        ...
        st.download_button(
            ...
            data=png,
            file_name="global_rm_schedule.png",
            mime="image/png",
        )