...
st.set_page_config(page_title="Global DM", layout="wide")

st.title("Global DM Scheduling")

render_sidebar("Global DM", show_protocol=True)
...
rows = render_task_inputs(
    ...
    key_prefix="global_dm",
)

st.subheader("Schedulability")
render_schedulability(rows, "Global DM", processors=int(processors))
...
if st.button("Generate Global DM schedule"):
    ...
    segments = simulate_global_dm(rows, int(range_end), int(processors))
    ...
    if png is None:
        ...
        st.download_button(
            ...
            data=png,
            file_name="global_dm_schedule.png",
            mime="image/png",
        )