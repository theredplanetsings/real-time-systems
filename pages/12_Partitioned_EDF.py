...
st.set_page_config(page_title="Partitioned EDF", layout="wide")

st.title("Partitioned EDF Scheduling")

render_sidebar("Partitioned EDF", show_protocol=True)
...
rows = render_task_inputs(
    ...
    key_prefix="partitioned_edf",
)

st.subheader("Schedulability")
render_schedulability(rows, "Partitioned EDF", processors=int(processors))
...
if st.button("Generate Partitioned EDF schedule"):
    ...
    if png is None:
        ...
        st.download_button(
            ...
            data=png,
            file_name="partitioned_edf_schedule.png",
            mime="image/png",
        )
