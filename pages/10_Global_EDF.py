...
st.set_page_config(page_title="Global EDF", layout="wide")

st.title("Global EDF Scheduling")

render_sidebar("Global EDF", show_protocol=True)
...
rows = render_task_inputs(
    ...
    key_prefix="global_edf",
)

st.subheader("Schedulability")
render_schedulability(rows, "Global EDF", processors=int(processors))
...
if st.button("Generate Global EDF schedule"):
    ...
    segments = simulate_global_edf(rows, int(range_end), int(processors))
    ...
    if png is None:
        ...
        st.download_button(
            ...
            data=png,
            file_name="global_edf_schedule.png",
            mime="image/png",
        )